...
frc2::JoystickButton(&joyLeft,3).WhenHeld(PIDShoot(&m_shooter, &m_intake)); // updated button
 ...
 frc2::JoystickButton(&joyRight, 11).WhenPressed(ThirdStageWheel(&cSensor)); // updated button
 frc2::JoystickButton(&joyLeft, 11).WhenHeld(shooterBackwards(&m_shooter));

 //Sydney spin wheel---------------------------------------------
 frc2::JoystickButton(&joyRight, 10).WhenHeld(SpinWheel(&cSensor));
}
